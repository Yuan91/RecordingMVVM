...
extension Item: IdentifiableType, Hashable {
    ...
    var changeObservable: Observable<()> {
        ...
        return NotificationCenter.default.rx.notification(Store.changedNotification).filter { [weak self] (notification) -> Bool in
            ...
            if let item = notification.object as? Item, item == s, !(notification.userInfo?[Item.changeReasonKey] as? String == Item.removed) {
                return true
            }
            else if let userInfo = notification.userInfo, userInfo[Item.changeReasonKey] as? Folder == s {
                return true
            }
            return false
        }.map { _ -> () in
